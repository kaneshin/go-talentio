module github.com/kaneshin/go-talentio

require (
	github.com/BurntSushi/toml v0.0.0-20160717150709-99064174e013
	github.com/davecgh/go-spew v0.0.0-20161029205726-346938d642f2
	github.com/google/go-querystring v0.0.0-20160401233042-9235644dd9e5
	github.com/k0kubun/pp v0.0.0-20160602144626-f5dce6ed0ccf
	github.com/mattn/go-colorable v0.0.0-20161103160040-d22884950486
	github.com/mattn/go-isatty v0.0.0-20160806122752-66b8e73f3f5c
	github.com/pkg/errors v0.8.0
	github.com/pmezard/go-difflib v0.0.0-20160110105554-792786c7400a
	github.com/stretchr/testify v0.0.0-20160925220609-976c720a22c8
	golang.org/x/sys v0.0.0-20161110161252-b699b7032584
)
